...
- **좌표** → 원본에 없다. 도로명주소 API로 생성했고 88/89건 확보됐다 (2026-09-07). 남은 1건은 군산진성교회이며 원본에 실재하는 번지가 없어 노회 문의가 남은 경로다.
...
- **모르는 것은 모른다고 말한다 — `data/notices.json`** (2026-09-07). 특정 교회의 데이터에 확인하지 못한 구멍이 있으면 그 상세 화면에서 미리 알린다. `import-source`가 `Church.notice`로 얹고 `ChurchNotice`가 그린다. 지금은 군산진성교회 1건이다.
