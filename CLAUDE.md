...
**`--brand-accent`는 `--accent`와 다른 것이다** (2026-09-08 추가). 청록 `oklch(0.48 0.12 195)`이고 `text-brand-accent`로 꺼내 쓴다. **`--accent`를 이 역할로 재정의하지 않았다** — 그쪽은 shadcn이 hover 배경으로 쓰는 토큰이라 색을 넣으면 버튼·드롭다운의 hover가 전부 청록이 된다.

- **청록은 두 번째 강조색이고, 역할이 둘이다** (2026-09-25 정리).
  - **지역 표시** — 홈 제목의 지역 롤링·`RegionTiles`·`/churches` 부제의 `N개 지역`·`지역` 필터 라벨·랜딩 요약의 `지역`/`시군구` 라벨·`ChurchCard`의 주소 핀.
  - **일반 강조** — `/about`의 인용·연표, `SectionTitle`의 번호 칩·진행 게이지, 홈 소개 카드 아이콘, 지도의 내 위치 점(네이비 이름표와 다른 종류라는 표시).
  - "청록 = 지역만"으로 좁히지 않기로 했다 — 일반 강조 넷을 걷어내면 `/about`과 지도를 손봐야 하고, 내 위치 점은 네이비와 갈릴 다른 색이 필요하다.
- ⚠️ **지역과 교단이 나란히 놓인 자리에서는 지역 쪽에만 쓴다.** 거기서는 청록이 둘을 가르는 신호다 — 홈의 교단 진입 줄이 청록을 피한 이유(`page.tsx`)이고, `ChipFilter`·`LandingFacts`가 **선택 prop으로** 지역 줄에만 청록을 주는 이유다.
- ⚠️ **네이비 면(`bg-primary`) 위에는 쓰지 않는다** — 대비가 무너진다. 홈 수록 현황 카드 안의 `16개 지역`이 청록이 아닌 이유다.
- ⚠️ **글자를 칠할 때는 라벨·짧은 수치까지만.** 값과 본문(주소 글자 등)까지 칠하면 읽는 글자가 강세색이 되어 위계가 뒤집힌다. 지역·교단 줄을 함께 쓰는 컴포넌트(`ChipFilter`·`LandingFacts`)는 **선택 prop으로 갈라** 교단 줄에 청록이 번지지 않게 했다.

**바탕은 순백이 아니다** (2026-09-25). `--background`·`--muted`·`--secondary`·`--accent`·`--border`·`--input`이 네이비(hue 250) 쪽으로 옅게 기울었고 **`--card`·`--popover`만 순백이다** — 카드가 바탕 위로 떠오르게 하려는 것이다. 대비를 잴 때 "흰 배경"을 가정하지 않는다.

- ⚠️ **같은 값으로 묶인 토큰이 두 벌 있고, 함께 움직인다.** 하나만 바꾸면 그 토큰을 쓰는 요소만 다른 회색으로 뜬다 — 2026-09-25에 두 번 다 실제로 밟았다.
  - `--secondary`·`--muted`·`--accent` — `--muted`만 옮겼다가 버튼·배지·칩만 순수 회색으로 남아 누렇게 떴다.
  - `--border`·`--input` — `--border`만 옮겼다가 검색창·제보 입력칸 테두리만 순수 회색으로 남았다(다크는 불투명도만 10%·15%로 다르다).
- 매니페스트의 `background_color`는 oklch를 못 읽어 `src/lib/site.ts`의 `BRAND_BACKGROUND`(hex)를 따른다 — **바탕을 바꾸면 거기도 고친다.**
...
- ⚠️ **파랑(250)과 청록(195)은 쓰지 않는다.** 이 사이트에서 그 둘은 이미 다른 뜻이다 — 네이비는 브랜드(`--primary`), 청록은 **지역 표시와 강조**(`--brand-accent`).
