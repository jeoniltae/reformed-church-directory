...
- **좌표** → 원본에 없다. 도로명주소 API로 생성했고 69/89건 확보됐다.
...
- `Church` 타입과 JSON 스키마는 **보유 89건 기준으로 확정됐다** (`src/types/church.ts`). 좌표(`lat`·`lng`)는 넣었고 `sns`·`worshipTimes`는 optional로 선언만 돼 있다(실데이터 0건).
