...
**바탕은 순백이 아니다** (2026-09-25). `--background`·`--muted`·`--secondary`·`--accent`·`--border`가 네이비(hue 250) 쪽으로 옅게 기울었고 **`--card`·`--popover`만 순백이다** — 카드가 바탕 위로 떠오르게 하려는 것이다. 대비를 잴 때 "흰 배경"을 가정하지 않는다.

- ⚠️ **`--secondary`·`--muted`·`--accent`는 같은 값이고 함께 움직인다.** `--muted`만 옮겼다가 버튼·배지·칩만 순수 회색으로 남아 누렇게 뜬 적이 있다.
