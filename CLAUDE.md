...
- **`SiteMark`는 되돌아가기 줄의 빈 오른쪽에 얹어 세로를 쓰지 않는다** (상세·지역·교단). 그 줄이 없는 화면(`/churches`·`/map`·`/report`·`/privacy`)만 제목 위에 한 줄을 쓴다 — **`/map`은 그 제목 줄 전체가 지도 위에 떠 있는 카드 안이다**(전체화면 지도라 세로를 쓸 수 없다, 2026-09-20). **홈·`/about`·`/not-found`에는 넣지 않는다** — 홈에는 로고 락업이 있고, `/about`은 **되돌아가기도 `SiteMark`도 `/`로 가서 같은 곳 링크가 둘이 되므로** 뺐다(2026-09-12). `/map`은 2026-09-08에 제외했다가 **지도가 붙어 제외 근거(중앙 정렬·`noindex`)가 사라져 2026-09-20에 넣었다.**
- **h1 급수는 화면 층위를 따른다** — 탭 루트(홈·`/churches`·`/map`)는 `t9`, 지역·교단 랜딩은 그 아래라 `t8`.
