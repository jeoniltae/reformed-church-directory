...
- **h1 급수는 화면 층위를 따른다** — 탭 루트(홈·`/churches`)는 `t9`, 지역·교단 랜딩은 그 아래라 `t8`. ⚠️ **`/map`의 h1은 `sr-only`다**(2026-09-22) — 전체화면 지도에 검색창까지 얹으면서 화면에서 뺐다. **지우지는 않는다**: 제목 없는 문서가 되면 8단계에서 색인을 열 때 그대로 드러난다.
