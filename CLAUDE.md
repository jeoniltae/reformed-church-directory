...
잠정 목록 — 보유 데이터를 확인한 뒤 확정한다. **[확인 필요]**
...
- **보유 데이터에 없는데 필요한 항목** → 2단계 개별 홈페이지 수집으로 보강한다. 예배시간·SNS가 대표적이며, 어떤 디렉토리성 소스에도 거의 없어 개별 홈페이지에서만 얻을 수 있다.
- **보유 데이터에만 있는 항목** (예: 소속 노회, 설립연도, 자매관계, 신조 채택 여부) → 확장 단계 소스에서 채울 수 없으므로 살릴지 버릴지 판단한다. 살린다면 optional 필드가 된다.
- **좌표** → 보유 데이터에 없을 가능성이 높다. 도로명주소 API로 생성한다.
