...
const DOES = [
  ...
  // 바로 아래 `DOESNT`의 "평가하거나 순위를 매기지 않습니다"를 **검증 가능하게**
  // 만드는 줄이다 (2026-09-12). 그 문장만 있으면 사용자는 목록 순서가 무엇인지
  // 알 수 없어 추천순으로 읽는다. 기준을 밝히면 화면에서 직접 확인할 수 있다.
  // ⚠️ 정렬 규칙을 바꾸면 이 문장도 함께 고친다 —
  // `src/features/churches/data.ts`의 `compareChurches`가 실제 구현이다.
  "목록은 지역·시군구·교회명 가나다순으로 보여줍니다 — 노출 순서에 어떤 평가도 반영하지 않습니다.",
];
      ...
      <main className="mx-auto w-full max-w-2xl flex-1 px-4 pt-4 pb-8">
        ...
        */}
        ...
        {/*
          ...
          **리드 문장은 두 번 바뀌었다.**
          ...
          ① 2026-09-12 — 첫 문장(`흩어진 개혁주의 교회 정보를 한곳에 모았습니다.`)은
          **바로 위 h1을 말만 바꿔 되풀이해 정보량이 0이었다.** 시안 문장
          (`낯선 도시에 도착한 성도가 가장 먼저 묻는 질문에…`)으로 바꿔 **읽는 사람을
          구체적으로 지목**하게 했다 — 홈 카드 부제를 `개혁주의란 무엇인지부터`로 잡은
          것과 같은 판단이다.

          ② 2026-09-22 — **만든 의도를 문장에 넣었다.** 그 전 두 문장은 **읽는 사람의
          상황(낯선 도시·이사)만 말하고 이 사이트가 왜 있는지는 말하지 않았다.**
          만든 의도는 이사가 아니라 **종교개혁의 신앙을 이어받은 한국 교회를 소개하고,
          그런 교회를 찾는 이들을 돕는 것**이다.

          ⚠️ **"성경적인 교회를 골랐다"로 쓰지 않는다.** 아래 `DOESNT`가
          **"교회의 신앙과 사역을 평가하거나 순위를 매기지 않습니다"**라고 못박았고
          수록 기준도 **교단 소속이지 개별 교회 심사가 아니다**(`DOES` 첫 줄).
          그래서 **우리가 판정한 것**이 아니라 **그 교회들이 스스로 잇고 있는 것**을
          말한다 — `종교개혁의 신앙을 이어받은`.

          ⚠️ **`모았습니다`를 쓰지 않는다.** 바로 위 태그라인이 이미 `한곳에`로 같은
          말을 한다 — 리드는 **누구를 위한 것인지**를 맡는다.

          ⚠️ **없는 기능을 약속하지 않는다.** 예배시간은 0건이라(`llms.txt`·위
          "사실과 다른 문구" 항목) 리드에서 언급하지 않는다.
        ...
        */}
        ...
        <p className="mt-2 text-t5 text-muted-foreground">
          종교개혁의 신앙을 이어받은 한국 교회들이 있습니다. 그 교회를 찾는 분들을
          위해 만들었습니다.
        </p>
