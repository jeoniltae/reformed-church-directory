...
/**
 * **template을 피하되 제목은 직접 쓴다 (2026-09-19).**
 *
 * 그전에는 `title`을 아예 선언하지 않고 `layout.tsx`의 `default`(사이트명)를
 * 상속받았다. **선언하면 template이 걸려 `홈 · 개혁주의 교회 디렉토리`가 되기
 * 때문**이었는데, 그 결과 홈 제목이 **12자**뿐이라 **Bing 웹마스터가
 * `Title too short`로 잡았다**(2026-09-19 실측).
 *
 * `absolute`**가 그 둘을 함께 푼다** — template을 우회하므로 접미사가 붙지 않고,
 * 서술을 더할 수 있다. **브랜드를 앞에, 설명을 뒤에** 두어 브랜드 쿼리와
 * `개혁주의 교회` 쿼리 양쪽에 걸리게 했다.
 *
 * ⚠️ **`추천`이라는 말은 이 사이트의 다른 원칙과 긴장 관계다.** `/about`은
 * **"평가하지 않습니다"**라고 적었고, 목록 정렬도 *"설명할 수 없는 순서는 추천순으로
 * 읽힌다"*는 이유로 기준을 세웠다(`features/churches/data.ts`). **사용자 판단으로
 * 넣었다** — 검색어로 실제 쓰이는 말이기 때문이다. **본문은 중립을 유지한다.**
 *
 * ⚠️ **`openGraph`는 건드리지 않는다.** 루트가 이미 홈을 가리키고 있고(og:url `/`),
 * 공유 카드의 제목은 **브랜드 그대로**인 편이 낫다.
 */
export const metadata: Metadata = {
  title: {
    absolute: "개혁주의 교회 디렉토리 · 국내 개혁주의 교회 추천 및 찾기",
  },
