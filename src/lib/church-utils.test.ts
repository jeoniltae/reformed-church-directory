...
describe("toAddressKeyword", () => {
  ...
  it("번호 중간에서 끊지 않는다", () => {
    // `13`까지만 먹으면 실재하는 다른 주소(창원 봉곡동 `사림로 13`)가 되어
    // 후보 38건이 쏟아지고 1순위가 엉뚱한 동네로 잡힌다. 실측으로 겪은 오답이다.
    expect(
      toAddressKeyword("경남 창원시 의창구 사림로 130번길 10-12 청호빌딩 301-501호"),
    ).toBe("경남 창원시 의창구 사림로 130번길 10-12");
    expect(toAddressKeyword("서울시 강남구 테헤란로 12길 5 3층")).toBe(
      "서울시 강남구 테헤란로 12길 5",
    );
  });
