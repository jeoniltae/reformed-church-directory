...
export function toAddressKeyword(raw: string): string {
  ...
  // **숫자가 와도 끊지 않는다.** "사림로 130번길"에서 `13`까지만 먹고 멈추면
  // 존재하는 다른 주소("사림로 13")가 되어 엉뚱한 동네를 정답처럼 물어온다.
  // 여기서 막으면 뒤로 물러나 "사림로 130번" + "길 10-12"로 맞춰진다.
  const m = v.match(/^(.*?(?:로|길|동|리|가)\s*\d+(?:-\d+)?)(?![가-힣\d])/);
