...
//
// ⚠️ **`/about`의 `06 이 사이트가 하는 일`이 아래 두 문장을 그대로 쓴다.**
// "한국 개신교 전체를 담는 디렉토리가 아니며…"와 "예배시간과 설립연도는…" 두 줄이다.
// 사이트 성격을 말하는 곳이 넷(여기·`DataNotice`·`/privacy`·`/about`)이라 문구가
// 갈라지기 쉬워 공유한다. **여기를 고치면 `src/app/about/page.tsx`의 `DOESNT`도
// 함께 고친다** — 특히 예배시간이 채워지는 날이 그렇다.
...
개혁신앙을 명확히 표방하는 교단과 독립개혁교회를 다룹니다 — 예장 고신·합신, 독립개신교회, 독립개혁장로회, 개혁교회 계열 등. **한국 개신교 전체를 담는 디렉토리가 아니며, 여기 없는 교회가 개혁주의가 아니라는 뜻도 아닙니다.** 범위에 맞는데 빠진 교회는 아래 제보 창구에서 등록 요청을 받습니다.
...
${link("소개", "/about", "이 디렉토리를 만든 이유, 개혁주의 신앙의 역사와 표준 문서, 용어 정리")}
...
${link("정보 수정·삭제·교회 등록 요청", "/report", "로그인 없이 보낼 수 있는 제보 창구")}
